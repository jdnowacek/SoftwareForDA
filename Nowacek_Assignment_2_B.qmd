...
```{r}
# implementation of renv snapshot, not necessary to run every time
...
```{r}
# when running this file, use this command to ensure package equivalence with
...
A fails to differentiate between character and non-character variables
B uses specific variable names, not as robust, does not follow the instructions
C targets the wrong variables
...
kableExtra::kable(table_data, digits = 2)
...
# this last line is commented since the question says "show code only"
```
